...
app.post("/issue", async (req, res) => {
  try {
    ...
    if (!preAuthGrant) {
      console.error("[/issue] Unsupported grant types. Expected pre-authorized_code. Found grants:", Object.keys(grants || {}));
    ...
    if (/^(openid-credential-offer:\/\/|haip:\/\/)/.test(deepLink)) {
      ...
      if (grants?.authorization_code) {
        if (!grants.authorization_code.issuer_state) {
          console.error("[/session] authorization_code grant missing required 'issuer_state' field");
          const failed = await setStatus("failed", { error: "authorization_code grant missing required 'issuer_state' field" });
          return res.status(400).json({ error: "invalid_grant", error_description: "authorization_code grant missing required 'issuer_state' field", state: failed });
        }
        try {
      ...
      console.error("[/session] No supported grant types found. Supported grants: urn:ietf:params:oauth:grant-type:pre-authorized_code, authorization_code (with issuer_state)");
      const failed = await setStatus("failed", { error: "No supported grant types found" });
      return res.status(400).json({ error: "unsupported_grant_type", error_description: "No supported grant types found. Supported grants: urn:ietf:params:oauth:grant-type:pre-authorized_code, authorization_code (with issuer_state)", state: failed });
    }
...
app.post("/issue-codeflow", async (req, res) => {
  try {
    ...
    const authGrant = grants?.authorization_code;
    if (!authGrant) {
      console.error("[/issue-codeflow] authorization_code grant type required in this endpoint. Found grants:", Object.keys(grants || {}));
    ...
    if (!authGrant.issuer_state) {
      console.error("[/issue-codeflow] authorization_code grant missing required 'issuer_state' field");
...
async function runPreAuthorizedIssuance({ apiBase, issuerMeta, configurationId, preAuthorizedCode, txCodeConfig, keyPath, pollTimeoutMs, pollIntervalMs, userPin }, logSessionId) {
  ...
  if (requestContext.credentialIdentifier) {
    console.log("[preauth] credential_identifier from token response:", requestContext.credentialIdentifier);
    try { slog("[preauth] credential_identifier", { credentialIdentifier: requestContext.credentialIdentifier }); } catch {}
  }
  ...
  const proofJwt = await createProofJwt({ privateJwk, publicJwk, audience: aud, nonce: c_nonce, typ: "openid4vci-proof+jwt", alg: selectedAlg });
  ...
  const credReq = { ...requestContext.requestPayload, proof: { proof_type: "jwt", jwt: proofJwt } };
  ...
  if (credRes.status === 202) {
    ...
    while (Date.now() - start < timeout) {
      ...
      if (defRes.ok) {
        ...
        await validateAndStoreCredential({ configurationId, credential: body, issuerMeta, apiBase, keyBinding: { privateJwk, publicJwk, didJwk }, metadata: { configurationId, credentialIdentifier: requestContext.credentialIdentifier, c_nonce }, storageKey: requestContext.storageKey }, logSessionId);
        return body;
      }
    }
  ...
  await validateAndStoreCredential({ configurationId, credential: credBody, issuerMeta, apiBase, keyBinding: { privateJwk, publicJwk, didJwk }, metadata: { configurationId, credentialIdentifier: requestContext.credentialIdentifier, c_nonce }, storageKey: requestContext.storageKey }, logSessionId);
  return credBody;
}
...
async function runAuthorizationCodeIssuance({ apiBase, issuerMeta, configurationId, issuerState, keyPath, pollTimeoutMs, pollIntervalMs }, logSessionId) {
  ...
  if (requestContext.credentialIdentifier) {
    console.log("[codeflow] credential_identifier from token response:", requestContext.credentialIdentifier);
    try { slog("[codeflow] credential_identifier", { credentialIdentifier: requestContext.credentialIdentifier }); } catch {}
  }
  if (c_nonce) {
...
async function validateAndStoreCredential({ configurationId, credential, issuerMeta, apiBase, keyBinding, metadata, storageKey }, logSessionId) {
  const slog = logSessionId ? makeSessionLogger(logSessionId) : (() => {});
  ...
  const credentialIdentifier = metadata?.credentialIdentifier;
  console.log("[validate] configurationId=", configurationId, "credentialIdentifier=", credentialIdentifier, "issuer=", issuerMeta?.credential_issuer, "has.c_nonce=", !!metadata?.c_nonce); try { slog("[validate] start", { configurationId, credentialIdentifier, issuer: issuerMeta?.credential_issuer, hasCNonce: !!metadata?.c_nonce }); } catch {}
  ...
  const storageIdentifier = storageKey || credentialIdentifier || configurationId;
  await storeWalletCredentialByType(storageIdentifier, { credential, keyBinding, metadata: { ...metadata, storageKey: storageIdentifier } });
}
