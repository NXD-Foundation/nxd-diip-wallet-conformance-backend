...
import { MDoc, Document, IssuerSignedDocument } from "@auth0/mdl";
...
function normalizeIssuerAuthHeaders(preparedIssuerSigned) {
  if (
    !preparedIssuerSigned ||
    !Array.isArray(preparedIssuerSigned.issuerAuth)
  ) {
    return;
  }
...
async function generateMdlCredentialManually(
  ...
  console.log(
    "Attempting to generate mDL credential using manual CBOR construction...",
  );
  try {
    ...
    if (!msoMdocClaims) {
      throw new Error(
        `Claims not found under namespace '${namespace}' for VCT: ${vct}. Available namespaces: ${Object.keys(claims.claims || {}).join(", ")}`,
      );
    }
    ...
    Object.entries(mDLClaimsMapped).forEach(([key, value], index) => {
      ...
      console.log(
        `Added claim: ${key} = ${value} (digestID: ${index}), encoded length: ${encodedItem.length} bytes, digest: ${digest.toString("hex").substring(0, 16)}...`,
      );
    });
    ...
    console.log(
      `MSO created for docType: ${docType}, namespace: ${namespace}, ${Object.keys(mDLClaimsMapped).length} claims`,
    );
    ...
    console.log(
      "MSO encoded (first 50 bytes):",
      encodedMSO.slice(0, 50).toString("hex"),
    );
    ...
    unprotectedHeadersMap.set(33, [
      Buffer.from(pemToBase64Der(issuerCertificateForSign), "base64"),
    ]); // x5c: certificate chain array - integer key 33

    console.log("COSE Headers Debug:");
    console.log(
      "Protected headers Map keys:",
      Array.from(protectedHeadersMap.keys()),
      "values:",
      Array.from(protectedHeadersMap.values()),
    );
    console.log(
      "Protected headers encoded length:",
      encodedProtectedHeaders.length,
      "bytes",
    );
    console.log(
      "Unprotected headers Map keys:",
      Array.from(unprotectedHeadersMap.keys()),
      "values types:",
      Array.from(unprotectedHeadersMap.values()).map((v) => typeof v),
    );
    ...
    try {
      ...
      console.log(
        "Decoded protected headers type:",
        decodedProtected.constructor.name,
      );
      if (decodedProtected instanceof Map) {
        console.log(
          "Decoded protected headers keys:",
          Array.from(decodedProtected.keys()),
        );
      } else {
        console.log(
          "Decoded protected headers keys:",
          Object.keys(decodedProtected),
        );
      }
    } catch (e) {
    ...
    console.log(
      `Creating signature over MSO (${encodedMSO.length} bytes) using ES256`,
    );
    ...
    if (signature.length !== 64) {
      console.warn(
        `⚠️ Warning: Signature length is ${signature.length} bytes, expected 64 bytes for ES256`,
      );
    } else {
      console.log(
        "✅ Signature created successfully (64 bytes, IEEE P1363 format)",
      );
    }
    ...
    console.log("COSE Sign1 structure types:", [
      ...
      typeof encodedProtectedHeaders,
      encodedProtectedHeaders.constructor.name,
      "unprotectedHeadersMap:",
      typeof unprotectedHeadersMap,
      unprotectedHeadersMap.constructor.name,
      "keys:",
      Array.from(unprotectedHeadersMap.keys()),
      "encodedMSO:",
      typeof encodedMSO,
      encodedMSO.constructor.name,
      "signature:",
      typeof signature,
    ...
    console.log(
      "COSE Sign1 wrapped in Tag 18 (COSE_Sign1_Tagged) for ISO/IEC 18013-5:2021 compliance",
    );
    ...
    try {
      ...
      if (
        decodedTest.issuerAuth instanceof cbor.Tagged &&
        decodedTest.issuerAuth.tag === 18
      ) {
        console.log(
          "✅ Verified: issuerAuth is properly wrapped in Tag 18 (COSE_Sign1_Tagged)",
        );
      } else {
        console.warn(
          "⚠️ Warning: issuerAuth Tag 18 verification failed. Expected Tag 18, got:",
          decodedTest.issuerAuth?.tag || typeof decodedTest.issuerAuth,
        );
      }
    } catch (e) {
    ...
    const encodedMobileDocument =
      Buffer.from(finalIssuerSigned).toString("base64url");
    console.log(
      "Manual mDL Credential generated (base64url):",
...
async function generateMdlCredentialWithAuth0Library(
  ...
  console.log(
    `[mdl-issue] Starting mDL credential generation using @auth0/mdl library for VCT: ${vct}`,
  );
  ...
  if (!credentialConfiguration) {
    const availableConfigs =
      Object.keys(
        issuerConfigValues.credential_configurations_supported || {},
      ).join(", ") || "none";
    console.error(
      `[mdl-issue] Configuration not found. Received VCT: '${vct}', Expected: one of [${availableConfigs}]`,
    );
    throw new Error(
      `Configuration not found for VCT: ${vct}. Received: '${vct}', Expected: one of [${availableConfigs}]`,
    );
  }
  ...
  if (!docType) {
    console.error(
      `[mdl-issue] doctype not defined. Received: undefined, Expected: a doctype string in credential configuration for VCT: ${vct}`,
    );
    throw new Error(
      `'doctype' not defined for VCT: ${vct}. Received: undefined, Expected: a doctype string`,
    );
  }
  const namespace = docType;
  ...
  if (!msoMdocClaims) {
    const availableNamespaces =
      Object.keys(claims.claims || {}).join(", ") || "none";
    console.error(
      `[mdl-issue] Claims not found under namespace. Received namespace: '${namespace}', Available namespaces: [${availableNamespaces}]`,
    );
    throw new Error(
      `Claims not found under namespace '${namespace}' for VCT: ${vct}. Received namespace: '${namespace}', Expected: one of [${availableNamespaces}]`,
    );
  }
  console.log(
    `[mdl-issue] Found claims under namespace '${namespace}': ${Object.keys(msoMdocClaims).length} claim(s)`,
  );
  ...
  const currentEffectiveSignatureType =
    ...
    sessionObject.signatureType === "x509"
      ...
      : "jwk";
  console.log(
    `[mdl-issue] Signature type determined: ${currentEffectiveSignatureType} (isHaip: ${sessionObject.isHaip}, ISSUER_SIGNATURE_TYPE: ${process.env.ISSUER_SIGNATURE_TYPE}, sessionObject.signatureType: ${sessionObject.signatureType})`,
  );

  // Step 4: Map claims to mDL format
  const mDLClaimsMapped = mapClaimsToMsoMdoc(msoMdocClaims, vct);
  console.log(
    `[mdl-issue] Mapped ${Object.keys(mDLClaimsMapped).length} claims to mDL format:`,
    Object.keys(mDLClaimsMapped),
  );
  ...
  if (!devicePublicKeyJwk) {
    console.error(
      `[mdl-issue] Device public key JWK not found. Received: ${JSON.stringify(cnf)}, Expected: cnf.jwk to contain a JWK object`,
    );
    throw new Error(
      `Device public key JWK not found. Received: ${JSON.stringify(cnf)}, Expected: cnf.jwk to contain a JWK object`,
    );
  }
  console.log(
    `[mdl-issue] Device public key JWK found: kty=${devicePublicKeyJwk.kty}, crv=${devicePublicKeyJwk.crv || "N/A"}`,
  );
  ...
  if (currentEffectiveSignatureType === "x509") {
    ...
    if (!issuerPrivateKeyForSign) {
      console.error(
        `[mdl-issue] Issuer private key (X.509) not found. Received: ${typeof issuerPrivateKeyForSign}, Expected: a PEM string`,
      );
      throw new Error(
        `Issuer private key (X.509) not found. Received: ${typeof issuerPrivateKeyForSign}, Expected: a PEM string`,
      );
    }
    if (!issuerCertificateForSign) {
      console.error(
        `[mdl-issue] Issuer certificate (X.509) not found. Received: ${typeof issuerCertificateForSign}, Expected: a PEM string`,
      );
      throw new Error(
        `Issuer certificate (X.509) not found. Received: ${typeof issuerCertificateForSign}, Expected: a PEM string`,
      );
    }
    console.log(
      `[mdl-issue] Issuer X.509 certificate loaded (length: ${issuerCertificateForSign.length} chars)`,
    );
  } else {
    console.log(
      "[mdl-issue] Using JWK for mDL signing (fallback to X.509 keys)",
    );
    issuerPrivateKeyForSign = privateKeyPemX509;
    issuerCertificateForSign = certificatePemX509;
  }
  ...
  console.log(
    `[mdl-issue] Validity period: ${validFrom.toISOString()} to ${validUntil.toISOString()}`,
  );

  try {
    console.log(
      `[mdl-issue] Adding ${Object.keys(mDLClaimsMapped).length} claims to namespace: ${namespace}`,
    );
    ...
    const document = await new Document(docType).addIssuerNameSpace(
      namespace,
      mDLClaimsMapped,
    );

    console.log(
      `[mdl-issue] ✅ Added issuer namespace with ${Object.keys(mDLClaimsMapped).length} claims`,
    );
    ...
    console.log(
      `[mdl-issue] issuerPrivateKey (PEM) type: ${typeof issuerPrivateKeyForSign}, length: ${issuerPrivateKeyForSign?.length || "N/A"}`,
    );
    console.log(
      `[mdl-issue] issuerCertificate (PEM) type: ${typeof issuerCertificateForSign}, length: ${issuerCertificateForSign?.length || "N/A"}`,
    );
    ...
    console.log(
      `[mdl-issue] Converting issuer private key from PEM to JWK format...`,
    );
    const { importPKCS8, exportJWK } = await import("jose");
    const issuerPrivateKeyObj = await importPKCS8(
      issuerPrivateKeyForSign,
      "ES256",
    );
    ...
    console.log(
      `[mdl-issue] ✅ Converted issuer private key to JWK format: kty=${issuerPrivateKeyJwk.kty}, crv=${issuerPrivateKeyJwk.crv || "N/A"}`,
    );
    ...
    console.log(
      `[mdl-issue] Sign options prepared: issuerPrivateKey (JWK), issuerCertificate (PEM), alg: ES256`,
    );
    console.log(
      `[mdl-issue] Signing document with issuer certificate (alg: ES256)...`,
    );
    ...
    console.log(
      `[mdl-issue] Preparing IssuerSigned structure for CBOR encoding...`,
    );

    const issuerSigned = signedDocument.issuerSigned;
    if (!issuerSigned || !issuerSigned.issuerAuth || !issuerSigned.nameSpaces) {
      throw new Error(
        "Signed document does not contain valid IssuerSigned structure",
      );
    }
    ...
    if (!preparedIssuerSigned) {
      throw new Error(
        "Prepared document does not contain issuerSigned structure",
      );
    }

    console.log(
      `[mdl-issue] IssuerSigned structure extracted from prepared document`,
    );
    console.log(
      `[mdl-issue] issuerAuth is array: ${Array.isArray(preparedIssuerSigned.issuerAuth)}`,
    );
    console.log(
      `[mdl-issue] nameSpaces is Map: ${preparedIssuerSigned.nameSpaces instanceof Map}`,
    );
    ...
    console.log(
      `[mdl-issue] Encoding IssuerSigned to CBOR using @auth0/mdl cborEncode...`,
    );
    const encoded = cborEncode(preparedIssuerSigned);
    console.log(
      `[mdl-issue] ✅ IssuerSigned encoded to CBOR (${encoded.length} bytes)`,
    );

    const encodedMobileDocument = Buffer.from(encoded).toString("base64url");
    console.log(
      `[mdl-issue] ✅ mDL Credential generated successfully using @auth0/mdl library`,
    );
    console.log(
      `[mdl-issue] Base64URL length: ${encodedMobileDocument.length} characters`,
    );
    console.log(
      `[mdl-issue] Base64URL preview (first 100 chars): ${encodedMobileDocument.substring(0, 100)}...`,
    );

    return encodedMobileDocument;
  } catch (error) {
    console.error(
      `[mdl-issue] ❌ Error during mDL credential generation:`,
      error.message,
    );
    console.error(`[mdl-issue] Error stack:`, error.stack);
    throw new Error(
      `Failed to generate mDL with @auth0/mdl library: ${error.message}. Received error: ${error.message}, Expected: successful credential generation`,
    );
  }
}
