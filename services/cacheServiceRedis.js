...
export async function statusListCreate(id, statusListObject) {
  try {
    if (!client.isReady) {
      console.log("Redis not ready, skipping statusListCreate");
      return false;
    }
...
export async function statusListGet(id) {
  try {
    if (!client.isReady) {
      console.log("Redis not ready, skipping statusListGet");
      return null;
    }
    const key = `status-lists:${id}`;
    const result = await client.get(key);
...
export async function statusListGetAllIds() {
  try {
    if (!client.isReady) {
      console.log("Redis not ready, skipping statusListGetAllIds");
      return [];
    }
...
export async function statusListDelete(id) {
  try {
    if (!client.isReady) {
      console.log("Redis not ready, skipping statusListDelete");
      return false;
    }
    const key = `status-lists:${id}`;
    await client.del(key);
...
export async function statusListUpdateIndex(id, index, status) {
  try {
    if (!client.isReady) {
      console.log("Redis not ready, skipping statusListUpdateIndex");
      return false;
    }
    const key = `status-lists:${id}`;
    const result = await client.get(key);
...
export async function statusListTryAllocateIndex(id, index) {
  try {
    if (!client.isReady) {
      console.log("Redis not ready, skipping statusListTryAllocateIndex");
      return false;
    }
